...
function Pokemon() {
    ...
    const { data: evochain, isLoading: evoLoading } = useSWR<EvoChainType>(species?.evolution_chain.url, http as Fetcher<EvoChainType>)

    const imgUrl = data?.sprites.other.dream_world.front_default ?? data?.sprites.other?.["official-artwork"].front_default ?? ""
    ...
    return (
        <Layout>
            <div className="w-full bg-base-100 rounded-xl shadow-xl px-6 h-12 flex items-center -mb-8">
            ...
            <div className="card lg:card-side bg-base-100 shadow-xl w-full">
                <figure className='z-10 h-80 w-80 cursor-pointer p-4 relative min-w-[30%]'>
                    {(imgUrl && !isLoading) ? (
                        <Image src={imgUrl} loading="lazy" alt='pokemon' onLoadingComplete={onImgCompleted} width={1} height={1} className={[imgLoaded ? 'scale-100 p-3 h-full w-auto' : 'scale-125 blur-2xl'].join(' ')} />
                    ) : (
                        <div className="animate-pulse flex space-x-4 w-full">
                                <div className="rounded-lg bg-gray-100 h-full w-full"></div>
                        </div>
                    )}
                </figure>
                <div className="card-body w-full">
                    {imgLoaded && <h1 className="card-title text-4xl">
                        {capitalize(pokename)}
                    </h1>}
                    ...
                    <p className={briefDesc?.includes('\n') ? "whitespace-pre break-words" : ""}>{briefDesc}</p>
                </div>
            </div>
            <div className='h-[360px] flex flex-col items-center relative gap-2 w-full'>
                ...
                <div className="flex">
                    {evoLoading && ((new Array(3).map((_, i) => (<CardLoader key={i} />))))}
